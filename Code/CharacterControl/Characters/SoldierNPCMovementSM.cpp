...
PE_IMPLEMENT_CLASS1(SoldierNPCMovementSM_Event_AIM, Event);
}
...
void SoldierNPCMovementSM::addDefaultComponents()
{
	...
	PE_REGISTER_EVENT_HANDLER(SoldierNPCMovementSM_Event_AIM, SoldierNPCMovementSM::do_SoldierNPCMovementSM_Event_AIM);
...
void SoldierNPCMovementSM::do_SoldierNPCMovementSM_Event_MOVE_TO(PE::Events::Event *pEvt)
{
	...
	OutputDebugStringA("PE:PROGRESS:SoldierNPCMovementSM::do_SoldierNPCMovementSM_Event_MOVE_TO():reveived event, running:");
...
void SoldierNPCMovementSM::do_SoldierNPCMovementSM_Event_AIM(PE::Events::Event* pEvt)
{
	...
	if (pSN)
	{
		...
		Events::SoldierNPCMovementSM_Event_AIM* pRealEvt = (Events::SoldierNPCMovementSM_Event_AIM*)(pEvt);
		Vector3 dir = pRealEvt->m_aimPosition - curPos;
		dir.normalize();
		// instantaneous turn
...
void SoldierNPCMovementSM::do_UPDATE(PE::Events::Event *pEvt)
{
	...
	//if (m_state == STANDING)
	//{
	//	//todo always rotate to target and aim it
	//	//need to find a way to get target position
	//	SceneNode* pSN = getParentsSceneNode();
	//	if (pSN)
	//	{
	//		Vector3 curPos = pSN->m_base.getPos();

	//		Vector3 dir = Vector3(float(rand()%10), 0.0f, float(rand() % 10)) - curPos;
	//		dir.normalize();
	//		// instantaneous turn
	//		pSN->m_base.turnInDirection(dir, 3.1415f);
	//		

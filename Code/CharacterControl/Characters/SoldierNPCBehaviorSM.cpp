...
void SoldierNPCBehaviorSM::do_SoldierNPCMovementSM_Event_TARGET_REACHED(PE::Events::Event *pEvt)
{
	...
	if (m_state == PATROLLING_WAYPOINTS)
	{
		...
		if (pGameObjectManagerAddon)
		{
			...
			{
				case 0:
					nextWayPointName = pWP->m_nextWayPointName;
					break;
				case 1:
					nextWayPointName = pWP->m_nextWayPointName1;
					break;
				case 2:
					nextWayPointName = pWP->m_nextWayPointName2;
					break;
				default:
					nextWayPointName = pWP->m_nextWayPointName;
					break;
			...
			if (pWP && StringOps::length(nextWayPointName) > 0)
			{
...
void SoldierNPCBehaviorSM::do_UPDATE(PE::Events::Event *pEvt)
{
	...
	if (m_state == IDLE)
	{
		...
		if (pGameObjectManagerAddon)
		{
			SoldierNPC* pSol = pGameObjectManagerAddon->getSoldierNPC();
			SceneNode * pSN = pSol->getFirstComponent<SceneNode>();
			PE::Handle h("SoldierNPCMovementSM_Event_AIM", sizeof(SoldierNPCMovementSM_Event_AIM));
			SoldierNPCMovementSM_Event_AIM* pEvt = new(h) SoldierNPCMovementSM_Event_AIM();
			pEvt->m_aimPosition = pSN->m_base.getPos();
